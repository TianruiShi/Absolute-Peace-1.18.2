...
public class StringodCoalWorldTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("stringod_coal_world_portal", Vec3i::compareTo, 300);
	public static PoiType poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("stringod_coal_world_portal",
				com.google.common.collect.Sets.newHashSet(
						ImmutableSet.copyOf(AbsolutepeaceModBlocks.STRINGOD_COAL_WORLD_PORTAL.get().getStateDefinition().getPossibleStates())),
				0, 1).setRegistryName("stringod_coal_world_portal");
	...
	public StringodCoalWorldTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getExitPortal(entity, blockpos1, worldborder).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = StringodCoalWorldPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return StringodCoalWorldPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d,
